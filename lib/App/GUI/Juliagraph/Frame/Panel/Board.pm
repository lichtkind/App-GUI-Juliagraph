...
sub paint {
    ...
    my $zoom_size = 4 * (10** (-$self->{'set'}{'constraints'}{'zoom'}));
    my $stop = $self->{'set'}{'constraints'}{'stop_value'};
    my $x_delta = $zoom_size;
    my $x_delta_step = $x_delta / $self->{'size'}{'x'};
    my $x_min = $self->{'set'}{'constraints'}{'pos_x'} - ($x_delta / 2);
    my $y_delta = $zoom_size;
    my $y_delta_step = $y_delta / $self->{'size'}{'y'};
    my $y_min = $self->{'set'}{'constraints'}{'pos_y'} - ($y_delta / 2);
    my $const_a = ($self->{'set'}{'constraints'}{'constant'} eq 'constant') ? $self->{'set'}{'constraints'}{'const_a'} : 0;
    my $const_b = ($self->{'set'}{'constraints'}{'constant'} eq 'constant') ? $self->{'set'}{'constraints'}{'const_b'} : 0;
    $const_a *= $factor{0}[0] if exists $factor{0} and $factor{0}[0];
    $const_b *= $factor{0}[1] if exists $factor{0} and $factor{0}[1];
    my $position = $self->{'set'}{'constraints'}{'position'};
    ...
    my $metric = { '|var|' => '($x*$x) + ($y*$y)', '|x*y|' => 'abs($x*$y)',
                   ...
                   '|x+y|' => 'abs($x+$y)',      '|x|+|y|' => 'abs($x)+abs($y)',
                    'x+y'  => '$x+$y',              'x*y'  => '$x*$y',
                    'x-y'  =>     '$x-$y',          'y-x'  => '$y-$x'}->{ $self->{'set'}{'constraints'}{'stop_metric'} };

    my @bg_color = ($self->{'set'}{'mapping'}{'color'} and $self->{'set'}{'mapping'}{'use_bg_color'})
                 ? color( $self->{'set'}{'mapping'}{'background_color'} )->values( in => 'RGB', as=>'list' )
                 : (0,0,0);
    ...
    my $colors = ($self->{'set'}{'mapping'}{'select'}-1) * ($self->{'set'}{'mapping'}{'gradient'}+1)
                 * $self->{'set'}{'mapping'}{'repeat'}    * $self->{'set'}{'mapping'}{'grading'};
    my @color = ();
    if ($self->{'set'}{'mapping'}{'color'}){
        $self->{'set'}{'color'}{ $self->{'set'}{'mapping'}{'select'} } = $self->{'set'}{'color'}{ 8 };
        for my $i (0 .. $self->{'set'}{'mapping'}{'select'} - 2) {
            my @gradient = map {[$_->values]}
                           color($self->{'set'}{'color'}{$i})->gradient( to => $self->{'set'}{'color'}{$i+1},
                                                                      steps => $self->{'set'}{'mapping'}{'gradient'}+2,
                                                                    dynamic => $self->{'set'}{'mapping'}{'dynamics'},
            );
            pop @gradient;
        ...
        @color = map {[$_->values]} color('white')->gradient( to => 'black',
                                                           steps => $self->{'set'}{'mapping'}{'select'} * ($self->{'set'}{'mapping'}{'gradient'}+2),
                                                         dynamic => $self->{'set'}{'mapping'}{'dynamics'},
        );
    }
    my $subgradient = int($self->{'set'}{'mapping'}{'grading'} > 1 and $self->{'set'}{'mapping'}{'grading_type'} eq 'Group');
    if ($subgradient){
        my @temp = @color;
        @color = ();
        for my $color (@temp){
            push @color, $color for 1 .. $self->{'set'}{'mapping'}{'grading'};
        }
    }
    if ($self->{'set'}{'mapping'}{'repeat'} > 1){
        my @temp = @color;
        @color = (@color, @temp) for 2 .. $self->{'set'}{'mapping'}{'repeat'};
    }
    if ($self->{'flag'}{'draw'}){
        $progress->add_percentage( $_ / $#color * 100, $color[$_] ) for 0 .. $#color;
        $progress->full;
    }
    if ($self->{'set'}{'mapping'}{'grading'} > 1 and $self->{'set'}{'mapping'}{'grading_type'} eq 'Sub' and not $self->{'flag'}{'sketch'}){
        for my $i (0 .. $#color){
            my $next = ($i == $#color) ? color( @bg_color ) : color( $color[$i+1] ) ;
            my @c = color( $color[$i] )->gradient( to => $next,
                                                   steps => $self->{'set'}{'mapping'}{'grading'}+2,
                                                   dynamic => $self->{'set'}{'mapping'}{'dynamics'},
                                                 );
            pop @c;
            $color[$i] = [@c];
    ...
    my $x_start_value = ($self->{'set'}{'constraints'}{'constant'} eq 'start value') ? $self->{'set'}{'constraints'}{'const_a'} : 0;
    my $y_start_value = ($self->{'set'}{'constraints'}{'constant'} eq 'start value') ? $self->{'set'}{'constraints'}{'const_b'} : 0;
  ...
  #  $code .= '        print " ",$v;'."\n";
    ...
    $code .= '      $img->SetRGB( $x_pix,   $y_pix,   @bg_color) if $i == $last_color;'."\n"
            if $self->{'set'}{'mapping'}{'use_bg_color'}; # and not $self->{'flag'}{'sketch'}
    $code .= '    }'."\n";
